...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 23
    answer = largest_number(([3, 4, 5, 6, 7, 7], [2, 3, 4, 5], [12, 12, 12, 23]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            if count == 0:
                if type(seq_seq[k][j]) == int or type(seq_seq[k][j]) == float:
                    largest = seq_seq[k][j]
                    count += 1
                    number = True
...
def run_test_largest_negative_number():
    ...
    expected = -7
    answer = largest_negative_number(([3, 4, 5, 6, -7, 7], [2, 3, 4, 5], [-12, 12, 12, 23]))
    print('Expected and actual are:', expected, answer)
